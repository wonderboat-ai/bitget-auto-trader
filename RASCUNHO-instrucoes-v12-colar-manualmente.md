# Projeto Auto-Trade — Instruções de Projeto (v12 — 2026-08-20)
...
**v12 herda a disciplina da v2-v11 — só atualiza os fatos, não a regra. "Status
atual" é uma FOTO (20/08, ~14:00 UTC), não uma promessa. O detalhe minuto a
minuto vive em `CLAUDE.md`, que muda mais rápido do que este documento deveria.
Substitui a v11 (colada em 19/08, a primeira desde a v7 — v8/v9/v10 nunca
chegaram a ser coladas).**

> **MUDANÇA ESTRUTURAL DESTA VERSÃO: este projeto (Bybit) está PAUSADO.** O
> motor foi parado deliberadamente em 20/08/2026 e **não deve ser religado
> nesta conta** — ver "Status atual" abaixo. Um projeto IRMÃO, clonado deste
> (mesmo código, histórico preservado, repositório GitHub separado) está sendo
> criado para portar o robô para a **Bitget**, indo direto para mainnet. Se
> você está numa sessão deste projeto (Bybit) e o pedido for sobre religar o
> motor, ligar `--live`, ou continuar a operação ao vivo, **pare e confirme
> com o Lucas antes** — o contexto operacional mudou.

## Status atual (fatos verificados até 2026-08-20, ~14:00 UTC)

- **NOVO (20/08): a conta Bybit que o robô operava caiu num conflito de
  identidade — o Lucas não consegue reautenticar o KYC porque alguém mais
  aparece associado ao CPF dele, com outro e-mail.** Isso é indício de
  possível fraude de identidade, fora do escopo de qualquer agente resolver —
  exige contato direto do Lucas com o suporte da Bybit (documento + selfie +
  disputa formal) e, possivelmente, um Boletim de Ocorrência. **O saldo real
  (~189 USDT) já foi retirado da Bybit e portado para a Bitget** pelo Lucas,
  antes de qualquer bloqueio pior.
- **Pesquisa confirmou que o problema não é específico da Bybit**: a
  CVM/Bacen está forçando toda exchange com entidade registrada no Brasil a
  bloquear derivativos (futuros/perpétuos) para residentes BR — **a OKX já
  faz o mesmo** (só spot/P2P liberado via entidade local). A Bybit anunciou
  liquidação forçada de posições incompatíveis a partir de **21/09/2026**.
  Binance e OKX estão descartadas como destino do robô pelo mesmo motivo.
  Exchanges offshore sem entidade BR registrada (Bitget, MEXC, Gate.io,
  KuCoin, BingX) ainda permitem derivativos pra brasileiros hoje — mas
  **exatamente por não seguirem o mesmo arcabouço regulatório que está
  fechando o cerco na Bybit**, o que é risco aceito conscientemente, não
  ausência de risco.
- **Decisão do Lucas: migrar o robô para a Bitget, direto para MAINNET (sem
  fase de testnet antes)** — desvio deliberado da prática histórica do
  projeto (testnet sempre primeiro). Avaliação técnica feita nesta sessão:
  Bitget foi escolhida como candidata de MENOR esforço de porte entre as
  opções offshore — tem conta unificada (UTA) parecida com a Unified Trading
  Account da Bybit v5, e `create_order` aceita `presetStopLossPrice`/
  `presetTakeProfitPrice` nativamente (melhor que a Bybit, que não tem OCO
  entre stop e TP). **Risco técnico conhecido e AINDA NÃO validado na
  prática**: o modo one-way (do qual o projeto inteiro depende, ver bug #7)
  tem gambiarras documentadas no ccxt especificamente pra Bitget (erro 40774
  no `create_order` padrão, workaround `side: "buy_single"/"sell_single"`).
- **O robô NÃO foi portado ainda** — nenhuma linha de `bybit_client.py` foi
  adaptada pra Bitget nesta sessão. O plano é **clonar este projeto inteiro
  num diretório novo + repositório GitHub novo** (preservando este projeto
  intacto como referência/frozen), com seu próprio CLAUDE.md/README/
  instruções, e portar a partir de lá — numa sessão nova, dedicada a isso.
- **Motor Bybit: PARADO deliberadamente em 20/08/2026, ~13:09:52 UTC**
  (`engine_stop`/manual, `CTRL_C_EVENT` limpo — desta vez funcionou de
  primeira porque o motor rodava num terminal externo com console real, não
  numa tarefa em background sem console). Nenhuma posição aberta no momento
  da parada. **Não religar nesta conta** até o Lucas decidir o que fazer com
  a disputa de KYC — mesmo que o motor volte a ser usado, é decisão dele,
  nunca automática.
- Todo o histórico anterior a 20/08 (22 dias de operação real, 53 trades,
  acerto 34,0%, PnL líquido ≈ −0,36 USDT, três rodadas de pesquisa sem edge
  validado, perfil 15m desligado, etc.) continua válido e não muda — ver
  seção "Status anterior (19/08)" logo abaixo, preservada como histórico.

### Status anterior (fatos verificados até 2026-08-19, ~20:00 UTC — histórico, ainda válido)
...
- **[NOVA PRIORIDADE, 20/08] Resolver a disputa de KYC/CPF na Bybit** — só o
  Lucas pode agir (documentos, suporte, possível B.O. de fraude de
  identidade). Bloqueante pra qualquer decisão sobre reaproveitar essa conta.
- **[NOVA, 20/08] Clonar o projeto pra um repositório novo e portar pra
  Bitget, mainnet direto.** Antes de escrever `bitget_client.py` de verdade,
  validar manualmente numa conta Bitget as 4 operações críticas: saldo
  unificado com campo de equity, ordem a mercado com SL/TP anexado
  (`presetStopLossPrice`/`presetTakeProfitPrice`), cancelamento seletivo, e
  **especialmente o modo one-way** (risco técnico conhecido, não validado —
  ver "Status atual"). Como vai direto pra mainnet sem testnet, cada uma
  dessas validações usa dinheiro real — ir com size mínimo até confirmar.
